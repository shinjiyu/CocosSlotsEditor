...
- **动画编辑**：每帧可配 `frameKind`（enter-table / reveal / highlight / postClear / compact / spinEnd）、动画模板（下落进场、重力落出、压缩下落、符号中奖、消除、脉冲、无动画）及其参数；支持「与上帧同播」并行转移
- **播放预览**：整段播放或单帧转移试播
...
- `assets/resources/symbol-library.prefab` 根节点挂 `SymbolLibrary` 组件，Inspector 中 +/− 增删 `SymbolEntry`，拖入纹理 / spine / 自定义 prefab 即完成配置
...
- **Symbol 包导出**：菜单「扩展 → 盘面编辑器 → 导出 Symbol 包」，从 symbol-library.prefab 出发收集依赖闭包（纹理、spine 全套、递归 prefab、运行时脚本、.meta），导出到 `temp/symbol-pack/`，整体合并进其它 Cocos 工程即可复用
...
assets/
  scripts/
    ...
    editor-app/         # 编辑器场景：BoardEditorMain / BoardView / EditorHud /
                        # SymbolLibrary / SymbolView / animTemplates / BoardDirector ...
    vendor/slot-presentation-ir/   # SPIR schema
  resources/
    symbol-library.prefab          # 符号库（IDE 中配置）
...
1. **配符号**：Creator 打开工程 → 双击 `assets/resources/symbol-library.prefab` → Inspector 中增删条目、拖资源、填动画名 → 预览墙实时查看效果
2. **编盘面**：预览运行编辑器场景（浏览器）→ 刷子绘制各帧盘面 → 配每帧 frameKind / 模板 / 参数 → 播放验证
