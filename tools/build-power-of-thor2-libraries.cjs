/**
 * 根据 manifest + Creator 生成的 .meta，写出 asset-library / symbol-library。
 *
 * 前置：已跑 import-power-of-thor2-pack.cjs，且 Creator 已 refresh 包目录。
 *
 * 用法：
...
function main() {
    ...
    for (const sp of manifest.spines) {
        ...
        const entry = assetEntryBase(sp.id, sp.id.replace(/^spine_thor2_/, ''), 1);
    ...
    // layout: 0 Prefab, 1 Node, 2 SymbolLibrary, then pairs of CellFxDef (win/vanish) per symbol, then SymbolEntry
    const symbolNodes = [];
    symbolNodes.push({
        __type__: 'cc.Prefab',
    ...
    const firstSymFx = 5; // each symbol: winFx, vanishFx, then later SymbolEntry block

    // We'll append: SymbolLibrary(2), global win(3), global vanish(4),
    // then for each symbol: winFx, vanishFx
    // then SymbolEntry objects
